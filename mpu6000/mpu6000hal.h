...
static __INLINE void mpu6000AssertCs(mpu6000Hal_t *hal)
{
	GPIO_ResetBits(hal->csGpioPort, hal->csGpioPin);
}

static __INLINE void mpu6000ReleaseCs(mpu6000Hal_t *hal)
{
	GPIO_SetBits(hal->csGpioPort, hal->csGpioPin);
}

static __INLINE void mpu6000DelayMs(uint8_t ms)
{
	uint32_t n = ms * 72000;
	while(n--)
		__NOP();
 ...
 */
static __INLINE bool mpu6000Read(mpu6000Hal_t *hal, uint8_t addr, uint8_t len, uint8_t *data)
{
	uint8_t n;
...
static __INLINE bool mpu6000ReadByte(mpu6000Hal_t *hal, uint8_t addr, uint8_t *data)
{
	/* Assert CS pin to start transmission */
	mpu6000AssertCs(hal);
...
static __INLINE void mpu6000Write(mpu6000Hal_t *hal, uint8_t addr, uint16_t len, uint8_t *data)
{
	uint8_t n;
...
static __INLINE void mpu6000WriteByte(mpu6000Hal_t *hal, uint8_t addr, uint8_t data)
{	
	/* Assert CS pin to start transmission */
	mpu6000AssertCs(hal);
...
static __INLINE bool mpu6000ReadBit(mpu6000Hal_t *hal, uint8_t addr, uint8_t bitNum, uint8_t *data)
{
	uint8_t byte;
...
static __INLINE bool mpu6000ReadBits(mpu6000Hal_t *hal, uint8_t addr, uint8_t bitStart, uint8_t length, uint8_t *data)
{
	uint8_t byte;
...
static __INLINE void mpu6000WriteBit(mpu6000Hal_t *hal, uint8_t addr, uint8_t bitNum, uint8_t data)
{
	uint8_t byte;
	mpu6000ReadByte(hal, addr, &byte);
...
static __INLINE void mpu6000WriteBits(mpu6000Hal_t *hal, uint8_t addr, uint8_t bitStart, uint8_t length, uint8_t data)
{
	uint8_t byte;
	uint8_t mask = ((1 << length) - 1) << (bitStart - length + 1);
	mpu6000ReadByte(hal, addr, &byte);
...
static __INLINE void mpu6000GpioInit(mpu6000Hal_t *hal)
{
...
static __INLINE void mpu6000IrqInit(mpu6000Hal_t *hal)
{
...
static __INLINE void mpu6000SpiInit(mpu6000Hal_t *hal)
{
 ...
 */
static __INLINE void mpu6000SetSpiFreq20M(mpu6000Hal_t *hal)
{
 ...
 */
static __INLINE void mpu6000SetSpiFreq1M(mpu6000Hal_t *hal)
{
	while(hal->spi->SR & SPI_I2S_FLAG_BSY);
 ...
 */
static __INLINE bool mpu6000GetIntPinStatus(mpu6000Hal_t *hal)
{
	return GPIO_ReadInputDataBit(hal->intGpioPort, hal->intGpioPin);
}

static __INLINE void mpu6000HalInit(mpu6000Hal_t *hal)
{
	mpu6000GpioInit(hal);
	mpu6000SpiInit(hal);
	mpu6000IrqInit(hal);
