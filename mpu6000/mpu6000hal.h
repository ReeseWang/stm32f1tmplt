...
typedef struct
{
	...
	uint32_t	irqGpioClk;	/* If not used, leave this variable unmodified (0x0000) */
...
void mpu6000GpioInit(mpu6000Hal_t *hal)
{
	...
	RCC_APB2PeriphClockCmd(hal->spiGpioClk | hal->irqGpioClk | \
	...
	/* Initialize IRQ GPIO Pin */
	if(hal->irqGpioClk != 0)
	{
		GPIO_InitStructure.GPIO_Pin = hal->irqGpioClk;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
		GPIO_Init(hal->irqGpioPort, &GPIO_InitStructure);
	}
}
